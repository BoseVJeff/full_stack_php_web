...
The system only has one major module - the \textbf{Files Module}.

This module is responsible for all file handling in the system.
It is divided into two parts - a database to store all metadata about each file, and a filesystem to store the files themselves.
The module was designed in this fashion to minimize filesystem access, which speeds up the overall system.

The other two modules in this system are the \textbf{User Module} and the \textbf{Subscription Module}.
The User Module is responsible for managing authentication and authorization of teh various users.
Additionally, it combines with the Subscription Module to manage subscription-based perks, like increased storage capacity.
...
\begin{itemize}
	\item \texttt{/api/files/}

	      Note that this endpoint requires an authenticated client.
	      All unauthenticated clients will recieve a \texttt{HTTP 401 Unauthorized} response.

	      \begin{itemize}

		      \item \texttt{PUT|POST}

		            Uploads the file to the associated user's account.

		            Care must be taken when using this endpoint to submit files, as it will create duplicate entries if the file already exists.
	      \end{itemize}
	...
	\item \texttt{/api/files/\textsl{<file>}}
	      ...
	      \begin{itemize}
		      ...
		      \item \texttt{POST}
		            ...
		            Replaces the current file with the provided file.

		            If a file is not included in the request, a \texttt{HTTP 400 Bad Request} response.
		            Consider using the \texttt{DELETE} method if the intent was to delete the file instead.

		            This endpoint requires an authenticated client.
		            All unauthenticated endpoints will recieve a \texttt{HTTP 401 Unauthorized} response.
	...
	\item \texttt{/api/user/config}
	      ...
	      \begin{itemize}
		      ...
		      \item \texttt{PATCH}
		            ...
		            Update the config with the new values supplied.
		      ...
		      \item \texttt{DELETE}
		            ...
		            Note that the system cannot delete a user's config without deleting the user itself.
		            Therefore, this action simply has the effect of resetting values back to default wherever possible.
	      \end{itemize}

	\item \texttt{/api/user}

	      This is the basic user endpoint.
	      All attempts to access this endpoint with an unauthenticated client will result in a \texttt{HTTP 401 Unauthorized} error.

	      \begin{itemize}
		      \item \texttt{DELETE}

		            Deletes the account associated with the client.
		            Note that this is a destructive action and cannot be reversed.

		            All files and user tokens associated with this account are permanently deleted.
	      \end{itemize}
\end{itemize}
...
\begin{longtblr}[
	...
	\hline
	\SetCell[r=4]{c} \texttt{/api/files/<file>} & [GET|POST|PATCH|DELETE] \texttt{<file>} does not exist                                                                                                                     & HTTP 404 Not Found                                                                                        & HTTP 404                                                                                                  \\
	\hline
	                                            & [GET|POST|PATCH|DELETE] \texttt{<file>} exists, user credentials invalid or missing                                                                                        & HTTP 401 Unauthorized                                                                                     & HTTP 401 Unauthorized                                                                                     \\
	\hline
	                                            & [GET|POST|PATCH|DELETE] \texttt{<file>} exists, user credentials valid, user not allowed to access file                                                                    & HTTP 401 Unauthorized                                                                                     & HTTP 401 Unauthorized                                                                                     \\
	\hline
	...
	\hline
	                                            & [POST] File not included in the request                                                                                                                                    & HTTP 206 Partial Content, with file data starting from offset \texttt{<start>} to the offset \texttt{end} & HTTP 206 Partial Content, with file data starting from offset \texttt{<start>} to the offset \texttt{end} \\
	\hline
	                                            & [POST] File included in request, \texttt{<file>} already exists                                                                                                            & \texttt{HTTP 200 OK} response, uploaded file replaces the old one                                         & \texttt{HTTP 200 OK} response, uploaded file replaces the old one                                         \\
	\hline
	% \end{tabular}
\end{longtblr}
...
Currently, all server administration is expected to be done by the systems administrator as a part of their regular sysadmin responsiblities.
Since this runs contrary to normal user expectations for such a service, a case could be made for having an explicit \textit{admin} user role.
In general, it could be a good idea to add user roles in the future, with associated permissions.
